...
export class VaultIndexManager {
  ...
  async load(): Promise<void> {
    try {
      ...
      this.vaultIndex = JSON.parse(data) as VaultIndex;
    } catch {
      this.vaultIndex = {};
    }
  }
  ...
  updateNote(
    noteName: string,
    variables: { [key: string]: VaultIndexValue }
  ): void {
  ...
  getNoteValue(noteName: string, variableName: string): VaultIndexValue | null {
    if (
      this.vaultIndex[noteName] &&
      this.vaultIndex[noteName][variableName] !== undefined
    ) {
